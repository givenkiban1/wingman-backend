...
async def test_root_is_up():
    from wingman.wingman_api.wing import api
...
async def test_root_returns_greeting():
    ...
    async with AsyncClient(app=api, base_url="http://test") as ac:
        response = await ac.get("/")
    assert response.json() == {"ok": "API connected"}
...
async def test_predict_has_key():
    ...
    async with AsyncClient(app=api, base_url="http://test") as ac:
        response = await ac.get("/predict", params=test_params)
    assert response.json().get('predicition', False)
...
async def test_predict_val_is_float():
    ...
    async with AsyncClient(app=api, base_url="http://test") as ac:
        response = await ac.get("/predict", params=test_params)
    assert isinstance(response.json().get('predicition'), float)
